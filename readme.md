...
开启后，让 Kilo、Cline、Cursor、Roo Code 等非官方客户端的上游请求尽量接近 Claude Desktop 和 Codex Desktop 直接使用该 API Key 时的 Header、Body、TLS 和路由形态。

#### 适用范围与触发条件

- 仅作用于明确启用开关的 Anthropic API Key 和 OpenAI Responses/Codex API Key 账号。
- 配置缺失或关闭时保持 CLIProxyAPI 现有行为；OAuth、xAI 和 OpenAI Chat Completions 兼容账号不进入该逻辑。
- 只有非官方客户端和账号连通性测试应用兼容 Profile；识别为官方 Claude/Codex Desktop 或 CLI 时跳过兼容改写，执行现有普通 API Key 链路。
- 官方客户端使用经抓包验证的身份信号正向识别；未明确识别的客户端按非官方客户端处理。该判断只控制请求改写，不作为鉴权边界。

#### 通用要求

- Claude 和 Codex 使用可版本化、不可原地变更的 Profile；启用后保存精确 Profile ID，未知、废弃或 Provider 不匹配的 Profile 必须拒绝，不得静默回退。
- Profile 契约必须按真实官方客户端的脱敏抓包冻结和维护；[sub2apiplus 的兼容实现](https://github.com/itv3/sub2apiplus/blob/main/README.md#11-api-key-官方客户端兼容)仅作为交叉验证，不得只复制单个 Header 或常量。
- Gateway 配置和 Profile 注册表是唯一事实源；Manager 只负责配置和展示状态，不参与普通模型请求。
- 先执行账号模型映射，再根据最终上游模型构造兼容请求。
- 账号连通性测试必须复用正式请求的 Profile、Header、Body、Transport 和路由构造链。
- 账号自定义 Header 不得覆盖 Profile 拥有的鉴权和身份 Header；该限制只作用于命中兼容 Profile 的请求。
- 页面、日志、错误、测试夹具和脱敏抓包不得暴露 API Key、OAuth Token、Cookie 或完整鉴权 Header。
...
- 只归一当前请求实际携带的已知工具，并使用请求级映射保持 JSON/SSE 响应中的工具调用可以正确对应；不得向普通请求强行注入完整工具基线。
- 区分固定身份 Beta 和根据请求内容动态增加的功能 Beta。
- 兼容 Profile 与现有 `applyCloaking` 不得叠加；未命中兼容 Profile 时保持原有 cloak 行为。
- 第一版保持 CLIProxyAPI 原有 Transport；后续版本只有在管理员明确选择已经抓包验证并注册的 TLS Profile 时才启用对应指纹。
...
- 第一版的 HTTP、SSE、WebSocket 和 TLS 选择遵循下方边界；后续扩展必须按对应 Profile 的真实抓包调整。

#### 第一版边界

- 仅提供 Claude Desktop 和 Codex Desktop Profile，不提供 Codex CLI Profile 选择。
- 非官方客户端命中 Codex Desktop Profile 时，上游使用 HTTP/SSE；下游 WebSocket 继续复用现有协议桥接，官方 Codex 客户端保留当前上游 WebSocket 行为。
- `tls-profile` 为空时使用现有 Transport；真实抓包确认前不提供或接受非空 TLS Profile。
- Manager 只在统一账号新增向导和编辑/详情弹窗中配置、展示状态；第一版不新增数据库字段、账号列表投影或旧 Provider 页面入口。
- 不修改 Translator、账号调度和模型映射，不复制隐藏 Prompt、产品 Memory 或账号状态，也不承诺 TLS 和 HTTP/2 帧级指纹完全一致。
...
- 同时验证配置关闭时的现有行为、xAI/OAuth 隔离、受保护 Header、精确 Profile 校验和敏感信息脱敏。

具体架构、配置契约、实施顺序和测试矩阵见 [API Key 官方客户端兼容实施方案](docs/api-key-official-client-compatibility-implementation-plan.md)。
